...
class TrainerTeacherStudent:
    ...
    def plot_student_test(self):
        ...
        for a in range(len(axes)):
            axes[a].bar(range(256), 
                        self.s_test_loss['teacher_latent_predicts'][imgs[a]], 'b', alpha=0.8, width=1, label='Teacher')
            axes[a].bar(range(256), 
                        self.s_test_loss['student_latent_predicts'][imgs[a]], 'orange', alpha=0.8, width=1, label='student')
